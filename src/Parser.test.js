...
describe('Parser', () => {
  ...
  describe('parse()', () => {
    ...
    it('should parse a spiders and list example', () => {
      const parser = new Parser('###', '>>>', '<<<');
      const results = parser.parse(loadFileData('example_with_spiders_links.txt'));
      spiders = {
      ...
      expect(results).to.have.property('Description').to.equal('Demo PR');
      expect(results).to.have.property('Spiders')
      expect(results).to.have.property('Links')
      assert.deepEqual(results['Spiders'], spiders)
      assert.deepEqual(results['Links'], links)
    });
  });
});
